...
const Dashboard = () => {
  ...
  const userChartData = useMemo(() => ({
    ...
    datasets: [
      {
        label: '구매 건수',
        data: userRanking.map(item => item.quantity),
        backgroundColor: [
          '#2563eb',
          '#3b82f6',
          '#60a5fa',
    ...
    <Container>
      ...
      <KpiGrid>
        <KpiCard>
          ...
          <KpiValue>{kpi?.totalSalesAmount ? kpi.totalSalesAmount.toLocaleString() : 0}원</KpiValue>
        </KpiCard>

        <KpiCard>
          <KpiLabel>📦 총 판매수량</KpiLabel>
          <KpiValue>{kpi?.totalQuantity ? kpi.totalQuantity.toLocaleString() : 0}개</KpiValue>
        </KpiCard>

        <KpiCard>
          <KpiLabel>🧾 총 주문건수</KpiLabel>
          <KpiValue>{kpi?.totalOrderCount ? kpi.totalOrderCount.toLocaleString() : 0}건</KpiValue>
        </KpiCard>

        <KpiCard>
          <KpiLabel>👥 고객 수</KpiLabel>
          <KpiValue>{kpi?.customerCount ? kpi.customerCount.toLocaleString() : 0}명</KpiValue>
        </KpiCard>

        <KpiCard>
          <KpiLabel>🛒 상품 수</KpiLabel>
          <KpiValue>{kpi?.productCount ? kpi.productCount.toLocaleString() : 0}개</KpiValue>
        </KpiCard>
      </KpiGrid>
